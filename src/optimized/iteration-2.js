...
// 8KB/198ms = 0.04KB/ms, 25x speed up required

// OPTIMIZED:
// chunk only unique subwords: 208ms average
// minimize Object.entries usage: 200ms average
// remove token reallocation: 198ms average
...
function trainBPE(corpus, separators, merges, vocab, inverse) {
    ...
    while(corpus.length > 1) {
        const shouldBeCorrect = strpbrk(corpus, separators);
        const chunk = corpus.slice(0, shouldBeCorrect);
        ...
        corpus = corpus.slice(shouldBeCorrect);
    }
    ...
    let ephemeralChunk;
    let ephemeralTokenArr = [];
    let previous = -1;
    for(let i = 0; i < tokens.length; i++) {
        if(previous !== i) {
            previous = i;
            ephemeralChunk = tokens[i];
            ephemeralTokenArr = [];
        } else {
            tokens[previous] = ephemeralTokenArr;
        }

        for(let j = ephemeralChunk.length; j > 0; j--) {
            const encoded = vocab[ephemeralChunk.slice(0, j)];
            if(encoded !== undefined) {
                ephemeralTokenArr.push(encoded);

                if(j !== 0) {
                    ephemeralChunk = ephemeralChunk.slice(j);
                    i--;
                }
    ...
    const chunkEntries = Object.entries(chunkFrequencies);
    for(let i = 0; i < merges; i++) {
        const freqs = {};
        for(let t = 0; t < tokens.length; t++) {
            for(let i = 0; i < tokens[t].length - 1; i++) {
                const pair = [
                    tokens[t][i],
                    tokens[t][i + 1],
                ];
                ...
                if(freqs[pair]) {
                    freqs[pair].frequency += chunkEntries[t][1];
                    freqs[pair].t.push(t);
                    freqs[pair].i.push(i);
                } else {
                    freqs[pair] = {
                        frequency: chunkEntries[t][1],
                        t: [t],
                        i: [i],
                    }
                }
        ...
        for(let i = 0; i < entries.length; i++) {
            if(entries[i][1].frequency > entries[max][1].frequency)
                max = i;
        }

        let pair = entries[max];
        if(
            (pair === undefined) ||
            (pair[1].frequency === 1)
        ) break;
        ...
        vocab[decoded] = VOCAB_SIZE;
        inverse[VOCAB_SIZE] = decoded;

        for(let x = 0; x < pair[1].t.length; x++) {
            const a = [];
            for(let i = 0; i < tokens[pair[1].t[x]].length; i++) {
                if(i === pair[1].i[x]) {
                    a.push(VOCAB_SIZE);
                    i++;
                } else a.push(tokens[pair[1].t[x]][i]);
            }

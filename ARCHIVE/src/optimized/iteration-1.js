...
// final: 295ms average
// 8KB/295ms = 0.027KB/ms, 37x speed up required

// OPTIMIZED:
// function inlining: 465ms average
// adjusted strpbrk logic: 455ms average
// avoid temp allocation in chunking: 440ms average
// cache tokens array outside of hot loop: 295ms average
...
function trainBPE(corpus, separators, merges, vocab, inverse) {
    ...
    while(corpus.length > 1) {
        // POTENTIAL FUNCTION CALL OVERHEAD
        const shouldBeCorrect = strpbrk(corpus, separators);

        // SLICE OVERHEAD
        chunked.push(corpus.slice(0, shouldBeCorrect));
